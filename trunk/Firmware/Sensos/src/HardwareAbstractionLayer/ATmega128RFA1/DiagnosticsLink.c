...
static uint8_t transmitterQueue[DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE];
static uint16_t transmitterQueueIn = 0;
static uint16_t transmitterQueueOut = 0;
static uint16_t transmitterQueueFree = DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE;
...
static uint8_t receiverQueue[DIAGNOSTICSLINK_RECEPTION_BUFFER_SIZE];
...
void DiagnosticsLink_Send(void* data, uint8_t length)
{
	...
	if (++transmitterQueueIn >= DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE)
	{
...
static void InterpretFrame()
{
	uint8_t length = receiverQueue[receiverQueueOut];
	if (++receiverQueueOut >= DIAGNOSTICSLINK_RECEPTION_BUFFER_SIZE)
	{
		receiverQueueOut = 0;
	}

	uint8_t frame[DIAGNOSTICSLINK_MAXIMUM_FRAME_SIZE];
	...
	switch (state)
	{
		case STATE_IDLE:
			if (udr == SOF)
			{
				...
				if (++receptionQueueInPosition == DIAGNOSTICSLINK_RECEPTION_BUFFER_SIZE)
				{
		...
		case STATE_RECEIVING:
			...
			else if (udr == EOF)
			{
				...
				if (transmitterQueueFree < DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE) // any frames waiting to be sent?
				{
					state = STATE_SEND_SOF; // tell transmitter to start sending a frame
				...
				EventDispatcher_Notify(InterpretFrame); // handle the received frame through a event notification
			}
			else
			{
		...
		case STATE_RECEIVING_ESC:
			...
			{
				receiverQueue[receptionQueueInPosition] = udr | 0x80; // unescape value and store it
		...
		case STATE_SEND_SOF:
			{
				...
				if (++transmitterQueueOut >= DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE) // remove length specifier from buffer
				{
		...
		case STATE_SEND_DATA:
			{
				...
				if (++transmitterQueueOut >= DIAGNOSTICSLINK_TRANSMISSION_BUFFER_SIZE) // remove data from buffer
				{
					transmitterQueueOut = 0;
				}
				bytesRemaining--;
		...
		case STATE_SEND_EOF:
			{
				...
				else
				{
					...
					SetBit(UCSR0B , TXCIE0); // enable TXC interrupt
				}
			}
			break;
